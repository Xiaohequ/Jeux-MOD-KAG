				...
				else vel = Vec2f(1,0);
				{
					...
					if (map.getHitInfosFromArc( this.getPosition()- Vec2f(2,0).RotateBy(-vel.Angle()), -vel.Angle(), 90, this.getRadius() + 16.0f, this, @hitInfos ))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							CBlob@ other = hi.blob;	  
							if (other !is null) // hit blob
							{
								if (other.hasTag("flesh") && other.getTeamNum() != this.getTeamNum()) // hit player
								{	
								...
								else if (!hit_block)
								{
									...
									{
										if(this.getHealth() < this.getInitialHealth()/4 && other.hasTag("zombie") && other.getTeamNum() == this.getTeamNum()){
											f32 power = this.get_f32("bite damage");
							...
							else //hit map
							{
								if ( hi.tile != CMap::tile_empty ) {		
									...
									if (!this.getMap().isTileGroundStuff( hi.tile ) && XORRandom(10) <= 5)
									{
										//if (XORRandom(2) == 0) 
										{
											this.getMap().server_DestroyTile(hi.hitpos, 0.3);
											this.set_u16("lastbite",0);
										}
									}
									else if(XORRandom(10) <= 5)
									{
										//if (XORRandom(2) == 0) 
										{
											this.getMap().server_DestroyTile(hi.hitpos, 0.2);
											this.set_u16("lastbite",0);
										}
									}
								}
