...
/* 	if (getRules().exists("ctf_costs_config")) {
	...
	// {
		// ShopItem@ s = addShopItem(this, "Burger - Full Health", "$quarters_burger$", "burger", "A burger to go.", false);
		// s.spawnNothing = true;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("shop made item")) {
		...
		string name = params.read_string();
		{
			...
			// else if (name == "burger") {
				// CBlob@ food = server_CreateBlob("food", -1, callerBlob.getPosition());
				// if(food !is null) {
					// server_Pickup(callerBlob, callerBlob, food);
				// }
			// }
		}
	}
