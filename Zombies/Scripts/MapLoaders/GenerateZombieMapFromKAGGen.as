...
bool loadMap( CMap@ _map, const string& in filename)
{
	...
	for(int i = 0; i < ruins_count; i++)
	{
		...
		s32 floor_height = 0;
		float rnddepth=ruins_random.NextFloat()+1.0;
		float underover = ruins_random.NextFloat();
		for(int x_step = 0; x_step < _size; ++x_step)
		{
			s32 _x = Maths::Min(width-1, Maths::Max(0,x + x_step));
			// if (underover > 0.8)
				// floor_height = Maths::Max( heightmap[_x] + 3, floor_height );	//ruins on the surface
			// else
				floor_height = Maths::Max( heightmap[_x] + 3 + int(rnddepth*deviation*1.4), floor_height );  //ruins underground = y
		}
		...
		for(int x_step = 0; x_step < _size; ++x_step)
		{
			...
			// naturemap[_x] = -1; // for ruins on the surface
			...
			for(int _upstep = 1;(_upstep < _upheight ||floor_height - _upstep + 1 > heightmap[_x] )&& _upoffset > 0;++_upstep)
			{
		...
		//Zombie portal and tech stuff spawn
		if (underover <= 0.8)
		{
			if (floor_height-12 > heightmap[x])
			{
				if (techstuff_spawn && scrolls_spawn)
				{
					...
					if (r == 4)
						...
						server_CreateBlob("ZombiePortal",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0));
				} else
				if (!techstuff_spawn && !scrolls_spawn)
				{
					server_CreateBlob("ZombiePortal",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0));
...
void SetupBackgrounds(CMap@ map)
{
	...
	map.AddBackground( "Sprites/Back/BackgroundPlains.png", Vec2f(0.0f, 10.0f), Vec2f(0.3f, 0.3f), color_white ); 
	map.AddBackground( "Sprites/Back/BackgroundTrees.png", Vec2f(0.0f,  29.0f), Vec2f(0.4f, 0.4f), color_white ); 
	//map.AddBackground( "Sprites/Back/BackgroundIsland.png", Vec2f(0.0f, 50.0f), Vec2f(0.5f, 0.5f), color_white ); 
	map.AddBackground( "Sprites/Back/BackgroundCastle.png", Vec2f(0.0f, 60.0f), Vec2f(0.6f, 0.6f), color_white ); 
