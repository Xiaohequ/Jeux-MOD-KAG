...
void Config(TDMCore@ this)
{
	...
	//how many kills needed to win the match
	this.kills_to_win = cfg.read_s32("kills_to_win", 20);
...
shared class TDMSpawns : RespawnSystem
{
	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
		if(getRules().isMatchRunning()){
			Vec2f[] respawnPlace;
			getMap().getMarkers("tdm2_spawn", respawnPlace );
			if (respawnPlace.length>0)
			{
				int checkRadius = 150;
				Vec2f[] canSpawnPlace;
				while(canSpawnPlace.length < 1 && checkRadius > 0){
					// printf("check player in radius");
					for(uint i = 0; i < respawnPlace.length; i++){
						Vec2f pos = respawnPlace[i];
						
						bool hasPlayer = false;
						CBlob@[] blob_around;
						getMap().getBlobsInRadius(pos, checkRadius, @blob_around);
						
						if(blob_around.length >0){
							for(uint i=0; i < blob_around.length; i++){
								if(blob_around[i].hasTag("player")){
									hasPlayer = true;
									break;
								}
							}			
						...
						if(!hasPlayer)
							canSpawnPlace.push_back(pos);
					}
					checkRadius -=20;
				}
				if(canSpawnPlace.length > 0){
					int spawnindex = XORRandom(997) % canSpawnPlace.length;
					return canSpawnPlace[spawnindex];
				}
			}
		}
...
shared class TDMCore : RulesCore
{
	...
	void updateHUD()
	{
		...
		serialised_team_hud.write_u16(0x5afe); //check bits // = access code: see TDM2_Interface.as, line 31 

		//for each team, serialise team kill info
		for (uint team_num = 0; team_num < teams.length; ++team_num )
		{
			TDM2_HUD hud;	// see TDM_Structs.as
			TDMTeamInfo@ team = cast<TDMTeamInfo@>( teams[team_num] );
		...
		//"transmit info to client" for make user interface, see TDM_interface.as
