...
shared class ZombiesSpawns : RespawnSystem
{
	...
	CBlob@ SpawnPlayerIntoWorld(Vec2f at, PlayerInfo@ p_info)
    {
        ...
        if (player !is null)
        {
			...
			if(player.lastBlobName != ""){
				info.blob_name = player.lastBlobName;
			}
			CBlob @newBlob = server_CreateBlob( info.blob_name, info.team, at );
            newBlob.server_SetPlayer( player );
			
			CBlob@ oldBlob = getBlobByNetworkID( info.spawn_point );
			if(oldBlob !is null){
				oldBlob.server_Die();
			}
			
			info.spawn_point = newBlob.getNetworkID();
			// printf("blob id "+ info.spawn_point);
    ...
    Vec2f getSpawnLocation(PlayerInfo@ p_info)
    {
		...
		ZSPlayerInfo@ c_info = cast<ZSPlayerInfo@>(core.getInfoFromName(p_info.username));
		if(c_info !is null)
        {
			// printf("spawn id "+ c_info.spawn_point);
			CBlob@ pickSpawn = getBlobByNetworkID( c_info.spawn_point );
			// printf("spawn id "+ c_info.spawn_point);
			//has spawn point
			if (pickSpawn !is null)
			{
				// printf("spawn id "+ c_info.spawn_point +" blob isnt null");
			...
			else //hasn't spawn point
			{
				...
				if(map !is null)
				{
					f32 x = XORRandom(2) == 0 ? 64.0f : map.tilemapwidth * map.tilesize - 64.0f;
    ...
    void AddPlayerToSpawn( CPlayer@ player )
    {
		...
		{
			error("PLAYER TEAM NOT SET CORRECTLY! Team: " + info.team);
		}
    }
	...
	s32 getPlayerSpawnTime(CPlayer@ player){
		ZSPlayerInfo@ info = cast<ZSPlayerInfo@>(core.getInfoFromPlayer(player));
		CBlob@ blob = getBlobByNetworkID( info.spawn_point );
		if(blob !is null && blob.hasTag("infected")){
			return 5 * getTicksASecond();
		}
